...
class VisDroneSSD2(nn.Module):
    def __init__(self, num_real_classes=None, img_size=1024):
        ...
        with torch.no_grad():
            ...
            # Layer4: 8x8 - для ОЧЕНЬ больших объектов
            self.layer4 = nn.Sequential(
                nn.Conv2d(f3.shape[1], 256, kernel_size=3, padding=1, stride=2),  # 16->8
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
        ...
        self.feature_maps_info = [
            ...
            {  # Level2: 64x64 - МЕЛКИЕ/СРЕДНИЕ объекты (20-100px)
                ...
                'sizes_px': [
                    ...
                    (130, 160),  # 145px
                    (160, 200),  # 180px
                    (200, 250),  # 225px - переход к Level3
                ]
            },
            {  # Level3: 32x32 - СРЕДНИЕ/КРУПНЫЕ объекты (80-250px)
                'size': (f3_h, f3_w), 
                'channels': f3.shape[1], 
                'num_anchors': 16,  # больше anchors для больших объектов
                'sizes_px': [
                    ...
                    # Диапазон 200-600px с упором на большие
                    (180, 220),  # 200px
                    (200, 250),  # 225px
                    (220, 280),  # 250px
                    (250, 300),  # 275px
                    (280, 320),  # 300px
                    (300, 350),  # 325px  <-- НАЧАЛО ОЧЕНЬ БОЛЬШИХ
                    (320, 380),  # 350px
                    (350, 400),  # 375px
                    (380, 430),  # 405px  <-- НАЧАЛО >400px
                    (420, 480),  # 450px
                    (450, 520),  # 485px
                    (480, 550),  # 515px
                    (520, 580),  # 550px
        ...
        self.analyze_anchor_distribution()
    ...
    def analyze_anchor_distribution(self):
        """Анализ распределения anchors по размерам"""
        print("\n" + "="*60)
        print("АНАЛИЗ РАСПРЕДЕЛЕНИЯ ANCHORS")
        print("="*60)
        
        # Преобразуем в пиксели
        boxes_px = self.default_boxes * self.img_size
        widths = boxes_px[:, 2] - boxes_px[:, 0]
        heights = boxes_px[:, 3] - boxes_px[:, 1]
        sizes = torch.sqrt(widths * heights)  # примерный размер
        ...
        for level_idx, info in enumerate(self.feature_maps_info):
            f_h, f_w = info['size']
            num_cells = f_h * f_w
            num_anchors_per_cell = info['num_anchors']
            level_anchors = num_cells * num_anchors_per_cell
            
            end_idx = start_idx + level_anchors
            level_sizes = sizes[start_idx:end_idx]
            
            print(f"\nУровень {level_idx} ({f_h}x{f_w}):")
            print(f"  Anchors: {level_anchors}")
            print(f"  Размеры: {level_sizes.min():.1f} - {level_sizes.max():.1f} px")
            print(f"  Медиана: {level_sizes.median():.1f} px")
            
            # Процент anchors больше 200px
            large_anchors = (level_sizes > 200).sum().item()
            print(f"  Anchors >200px: {large_anchors} ({large_anchors/level_anchors*100:.1f}%)")
            
            start_idx = end_idx
        
        # Общая статистика
        print(f"\nОБЩАЯ СТАТИСТИКА:")
        print(f"Всего anchors: {len(sizes)}")
        
        size_bins = [0, 50, 100, 200, 300, 400, 600]
        for i in range(len(size_bins)-1):
            min_sz, max_sz = size_bins[i], size_bins[i+1]
            count = ((sizes >= min_sz) & (sizes < max_sz)).sum().item()
            percent = count / len(sizes) * 100
            print(f"  {min_sz}-{max_sz}px: {count} ({percent:.1f}%)")
        
        # # Визуализация
        # plt.figure(figsize=(12, 5))
        # plt.subplot(1, 2, 1)
        # plt.hist(sizes.numpy(), bins=50, alpha=0.7, color='blue')
        # plt.title('Распределение размеров anchors')
        # plt.xlabel('Размер (px)')
        # plt.ylabel('Количество')
        # plt.axvline(x=200, color='red', linestyle='--', label='200px')
        # plt.axvline(x=300, color='orange', linestyle='--', label='300px')
        # plt.legend()
        
        # plt.subplot(1, 2, 2)
        # plt.scatter(widths.numpy(), heights.numpy(), alpha=0.1, s=1)
        # plt.title('Соотношение ширина/высота anchors')
        # plt.xlabel('Ширина (px)')
        # plt.ylabel('Высота (px)')
        # plt.plot([0, 600], [0, 600], 'r--', alpha=0.5)
        # plt.xlim(0, 600)
        # plt.ylim(0, 600)
    ...
    def nms(boxes, scores, threshold=0.5):
        ...
        while len(boxes) > 0:
            ...
            ious = VisDroneSSD2.calculate_iou(boxes[0:1], boxes[1:]).squeeze()
            mask = ious <= threshold
            boxes = boxes[1:][mask]
